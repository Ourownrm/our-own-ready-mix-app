...
// one rate per scope — see schema.sql's comment on rm_supplier_rates.
router.get("/suppliers/:supplierId/rates", requireRole(...ORDER_ROLES), async (req, res) => {
  const { rows } = await query(
    `SELECT sr.*, m.name AS material_name, m.purchase_unit
     ...
     WHERE sr.supplier_id = $1 AND sr.is_active
...
router.post("/suppliers/:supplierId/rates", requireRole(...ADMIN), async (req, res) => {
  const { material_id, scope, rate } = req.body;
  ...
  const { rows } = await query(
    `INSERT INTO rm_supplier_rates (supplier_id, material_id, scope, rate, updated_by)
     VALUES ($1,$2,$3,$4,$5)
     ON CONFLICT (supplier_id, material_id, scope)
     DO UPDATE SET rate = EXCLUDED.rate, is_active = true, updated_by = EXCLUDED.updated_by, updated_at = now()
     RETURNING *`,
    [req.params.supplierId, material_id, scope, rate, req.user.id]
  );
  res.status(201).json(rows[0]);
});
