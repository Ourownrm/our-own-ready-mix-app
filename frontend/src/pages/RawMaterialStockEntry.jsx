// Raw material stock entry — moved from QC Engineer to Lab Technician
// (round 118 refinement, per explicit request). QC Engineer no longer has
// write access to this screen at all; Lab Technician is now the only role
// that can save stock levels (Manager/Administrator still see it read-only
// via RawMaterialStockCard.jsx on their dashboards, unchanged).
import { useEffect, useState } from "react";
import { TopBar } from "../lib/TopBar.jsx";
import { apiRequest } from "../lib/api.js";

export default function RawMaterialStockEntry() {
  ...
  async function save() {
    ...
    // screen open on a shared device — only whoever knows the PIN can
    ...
    try {
      const updated = await apiRequest("/lab-technician/raw-material-stock", {
        method: "PUT",
  ...
  return (
    <>
      <TopBar title="Lab Technician · Raw Material Stock" />
