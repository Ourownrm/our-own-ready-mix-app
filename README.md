...
## Round 129 (Ver. 9.49): Director dashboard Order Qty fix, truck tracking for Site Supervisor/Sales, cube-test edit access for Lab Technician, three mockups

Four items from a fresh feedback list: one dashboard-number fix, one access-widening change, one
role-access change, and three design-only mockups for a follow-up round (explicitly requested as
mockups, not code, so nothing under item 4 below shipped as working code this round).

1. **Director Dashboard's "Order Qty Today" was undercounting real demand.** The KPI's SQL already
   excluded cancelled/closed orders (so `completed`/`in_progress`/`planned` orders were never the
   problem on paper), but validated against the actual business rule with the user first rather than
   guessing: a cancelled or closed order that never received any real supply should be excluded (it
   was never real demand), but a cancelled/closed order that DID receive some supply before being
   cancelled/closed should still count, same as any completed or still-open order. Fixed
   `reports.js`'s `director-dashboard` route: the exclusion for `cancelled`/`closed` orders is now
   conditional on there being no non-cancelled `delivery_tickets` row against that order (an `EXISTS`
   subquery), instead of a blanket status exclusion.
2. **Truck tracking (previously Manager/Administrator-only, from an order's detail view) opened up to
   Site Supervisor and Sales Executive too**, each scoped to their own orders. Backend:
   `GET /orders/:orderId/tracking` now also accepts `site_supervisor` (scoped to
   `assigned_site_supervisor_id`) and `sales_executive` (scoped through the same
   `COALESCE(site's assigned_sales_representative_id, order's sales_representative_id)` pattern used
   everywhere else in `sales.js`), each failing closed with a plain 404 on an order outside their
   scope — no information leak about someone else's order. A new `sales.js` route,
   `GET /sales/my-orders-today`, gives Sales Executive their own today's-orders list to track from
   (Site Supervisor already had one). Frontend: the `TruckTrackingPanel` component moved out of
   `OrderDetailModal.jsx` (where it was a private, unexported component) into the shared
   `DeliveryTrackingView.jsx`, so `SiteSupervisor.jsx` and `SalesExecutive.jsx` can both reuse it —
   each order card in Site Supervisor's "Today's orders" and Sales Executive's new "Today's orders"
   card gets a "Track all trucks for this order" toggle.
3. **Lab Technician granted date-change and delete access on recorded cube test results** (both
   plant/pour-level and site-cast), previously Administrator-only since Round 124/125. They're the
   ones entering these results in the first place, so requiring an Administrator to fix a data-entry
   mistake was friction without a real safeguard behind it — Lab Technician can already overwrite a
   result's values (date included) via the existing save action. Backend: `requireRole("administrator")`
   widened to `requireRole("lab_technician", "administrator")` on the plant-result delete, the
   site-cast-result delete, and the plant-result date-change PATCH (no equivalent date-change route
   exists for site-cast results — nothing to widen there). Frontend: `LabTechnician.jsx`'s
   `isAdmin`-gated Delete/Change date buttons (in both `PourDetail` and `SiteCastDetail`) now show for
   `canManageResults` (administrator or lab_technician) instead — the frontend/backend role-guard
   mismatch this app has hit before (see the Project's own instructions doc) checked both ways this
   time.
4. **Three items delivered as mockups only, per explicit instruction ("Need Mock-up for below before
   coding") — no working code shipped for any of these this round:**
   - Customer module: an order card showing Order/Delivered/Completed quantity with a progress bar;
     the live tracking page showing overall order status prominently above the truck list; and a
     "delivery summary" view (truck, quantity, completion time) replacing the tracking page's current
     blank/error state once every truck has finished delivering.
   - Lab Technician module: a combined cube test report mockup that omits any cube sampled-but-never-
     tested row entirely (currently shown blank) and, once both 7-day and 28-day testing are done for
     a pour, combines both into one report — 7-day rows first, then 28-day rows, each with its own
     computed average row, plus new Date of Test and Age (Days) columns; and a Site Supervisor
     addition to the existing truck-completion form letting them record whether cubes were prepared
     at site and how many samples were taken.
   - A new Truck Timing Report mockup for the Manager Dashboard, structured like the existing
     "Completed trips today" table but as its own filterable (date-range) report page, adding the one
     column Completed Trips doesn't have: Plant In (`returned_to_plant`) alongside Batch, Plant Out,
     Site In, Unloading Start, and Site Out.

Verified with `node --check` on `reports.js`, `orders.js`, `sales.js`, and `labTechnician.js`, and a
clean `npm run build`. The three mockups (six artboards total) were built to match the real app's
design tokens and component markup (`index.css`, `DeliveryTrackingView.jsx`'s `TruckCard`,
`cubeTestPdf.js`'s report layout, `SiteSupervisor.jsx`'s `CompleteForm`, `ManagerDashboard.jsx`'s
`CompletedTripsTable`) rather than a fresh visual style, then reviewed for genuine layout/contrast
bugs and against the request — two real issues turned up (a low-contrast progress-bar segment, and
average rows in the combined cube report that were label-only instead of showing computed per-column
values) and were fixed before final delivery.

## Round 130 (Ver. 9.50): Round 129's three mockups shipped as working code

Round 129's item 4 mockups went through two further rounds of design-only feedback (cube report
header/table/summary layout, site-cube-recording independence, order-status/tracking-summary
restoration, truck display order) before this round's "ok proceed to code" — everything below is the
FINAL, feedback-revised shape of those mockups, not the original Round 129 draft.

1. **Customer module — delivery progress card, tracking-screen order status, post-completion
   summary.** `customerPortal.js`'s `GET /orders/:id` now returns two distinct quantities instead of
   one: `dispatched_qty_m3` (everything dispatched so far, net of rejections, including a truck still
   on site — same formula `orders.js`/`deliveryConfirmation.js` already use) and `delivered_qty_m3`
   (unchanged in meaning — only trucks that have actually finished unloading). `CustomerPortal.jsx`'s
   order detail screen replaces the old single "Delivered so far: X m³" line with a new
   `DeliveryProgressCard` — Ordered/Delivered/Completed stat trio plus a layered progress bar,
   sitting alongside the existing (unchanged) 4-stage order-status stepper. On the live-tracking
   screen (`OrderTrackingScreen`), an order-status banner now sits above the truck list in both
   states, and the screen itself now recognizes a terminal order (`getOrderTrackingPayload` — used by
   this screen, the staff tracking route, and the legacy public link — now also returns
   `terminal_at`) and switches to a "Delivery summary" framing instead of erroring out or looking
   identical to the live view. The "Track this delivery" button on order detail now also shows once
   an order is completed/closed (relabeled "View delivery summary"), since the tracking payload stays
   available for `TRACKING_GRACE_HOURS` after completion — it just had no way in to reach it before.
   Truck display order needed no change — `orderTracking.js` already lists newest-truck-first
   (Round 120), which is what "first truck should go bottom" asks for; verified, not touched.
2. **Lab Technician — combined cube test report rebuilt to the final mockup**, replacing the
   per-result-only `renderCubeTestSection` output for this one use case with a new one-page combined
   layout (`renderCombinedPourSection`/`generateCombinedPourCubeTestPdf` in `cubeTestPdf.js`): no
   Sample IDs in the header (Casting Date takes that slot instead), the STANDARDS FOLLOWED section
   restored, no footnote for an untested cube (it's simply not a row, same as before), one CUBE TEST
   ANALYSIS table spanning both ages with a rowspan-style Average column on the right instead of
   full-width "AVERAGE — 7/28 DAY" label rows, a two-column TEST SUMMARY panel (load/density sit
   beside the headline strength number, not stacked under it) with the f'ck pass/fail line moved into
   the navy bar itself, and Tested By/Checked By side by side on one row well clear of Remarks. New
   backend routes `GET /lab-technician/cube-pours/:orderId/combined-pdf-data` and
   `GET /lab-technician/site-cube-casts/:castId/combined-pdf-data` assemble both ages' results (and
   their cubes) for one pour/cast; a new `GenerateReportBar` control (shown only once both ages are
   tested — before that there's just the one result, same as before) offers **Combined (both ages)**,
   **7-Day only**, or **28-Day only** in both `PourDetail` and `SiteCastDetail`, directly answering
   the explicit ask for that choice. The old `generateCombinedCubeTestPdf` (separate full reports
   concatenated, one per same-day result — a different feature, for possibly-different pours landing
   on the same calendar day) is untouched.
3. **Site Supervisor — cube samples recordable independently of the delivery-confirmation buttons.**
   A new "Cubes prepared at site" card (`SiteCubeRecordingCard`) sits as its own sibling card below
   the existing per-truck delivery card — its own Yes/No toggle, cube-count input, and Save action
   that touches nothing else on the screen. Backend: `siteSupervisor.js` gets
   `POST /:ticketId/site-cubes` (resolves the ticket to its order and upserts *this supervisor's own*
   entry for *today*, into the same `site_cube_casts` table Lab Technician's own recording already
   uses — an advisory lock around the check-and-write closes the double-tap race a plain
   select-then-write would otherwise leave open) and `GET /:ticketId/site-cubes/today` (so the card
   can show "already saved once today: N cubes at HH:MM" before the supervisor even taps Save). Goes
   through the same offline-queue-aware `act` helper the arrival/unloading/reject buttons use
   (`act` now returns its response instead of discarding it, so the card can reflect what was actually
   saved — or, offline, that it's queued).
4. **Lab Technician — site-recorded samples now surface in the pour's own sample list, labeled "At
   Site".** `GET /lab-technician/cube-pours/:orderId` now also returns `site_casts` (any
   `site_cube_casts` rows against that same order), and `PourDetail`'s "Cube samples for this pour"
   list renders them alongside the plant DNs — "Plant" badge for DN rows, new "At Site" badge
   (`.badge-violet`, added to `index.css`) for these, each pointing at the separate Site-Cast Cubes
   tab where their results are actually entered (a genuinely separate table/form underneath, per
   Round 121's original design — this is a visibility fix, not a merge of the two entry flows).
5. **New Truck Timing Report for Manager Dashboard** (`TruckTimingReport.jsx`, routed at
   `/truck-timing-report`, linked from the dashboard's Reports menu). Same trips as the dashboard's
   own Completed Trips table (`status IN ('completed','rejected')`), but filterable by date range
   (`GET /orders/truck-timing-report?from=&to=`, defaulting to the last 5 days) instead of always
   today, and with the full round-trip timeline — Batch, Plant Out, Site In, Unloading Start, Site
   Out, Plant In — using the same canonical `trip_events` types every other timing view in this app
   already agrees on (`left_plant` for Plant Out specifically, not Completed Trips' own use of the
   earlier QC-release `dispatched` event for that column).

A background review pass (before delivery) on top of the usual `node --check`/`npm run build` caught
two real issues, both fixed: the site-cube upsert's race window (item 3, closed with the advisory
lock rather than a schema constraint, since a hard UNIQUE would have started rejecting Lab
Technician's own legitimate multiple-casts-per-day pattern); and a non-deterministic result pick in
the two new combined-pdf-data routes for old (pre-Round-122) legacy multi-DN pours that could have
more than one same-age result on file — `ORDER BY testing_age_days, tested_at DESC` now picks the
most recent one deterministically instead of whichever the query planner happened to return first.
A third issue turned up on a self-check against this project's own documented canonical
`trip_events` mapping after this section was first drafted: item 5's "Site Out" column was querying
`left_site`, which (per the existing round-98 postmortem comment in `maintenance.js`) is only an
unconfirmed geofence hint, not a real driver-confirmed event — every other timing view in the app
